...
import jp.ac.tokushima_u.is.ll.dao.LanguageAbilityDao;
...
public class ItemQueueService {
	...
	@Autowired
	private LogUserReadItemDao readItemDao;
	@Autowired
	...
	public void updateItemQueue(String itemId, String userId, int queuetype) {
		List<QuestionType> questionTypes = questionTypeDao.findListByItemId(itemId);
...
//		criteria.addOrder(Order.desc("startTime"));
		...
		List<ItemQueue> queues = this.itemQueueDao.findListByUserAndItem(userId, itemId);
		
		if (queues != null && queues.size() > 0) {
			for (ItemQueue queue : queues) {
				queue.setDisabled(1);
				queue.setUpdateTime(new Date());
				this.itemQueueDao.update(queue);
			}
		}
...
//			read_dc.add(Restrictions.eq("item", item));
			List<LogUserReadItem> results = this.readItemDao.findListByUserAndItem(userId, item.getId());
			if (results.size() > 2) {
				itemqueue.setWeight(Constants.PriorityReferItemWeight);
			} else
				itemqueue.setWeight(Constants.PriorityRecommendedItemWeight);
		}
		itemqueue.setId(KeyGenerateUtil.generateIdUUID());
		this.itemQueueDao.insert(itemqueue);
	}
...
//		String study_lans = "";
		List<Language> studyLangs = languageDao.findListUsersStudyLangs(user.getId());
		if(studyLangs == null || studyLangs.isEmpty()){
			return null;
		}
		
//		for (int n = 0; n < studyLangs.size(); n++) {
//			if (n > 0)
//				study_lans += ",";
//			study_lans = study_lans + " '"
//					+ studyLangs.get(n).getId() + "' ";
//		}
//		if (study_lans.length() <= 0)
//			return null;
//		
//		String my_lans = "";
		List<Language> myLangs = languageDao.findListUsersMyLangs(user.getId());
		if(myLangs == null || myLangs.isEmpty()){
			return null;
		}
...
//				.addEntity(Item.class).list();
		List<Item> items = itemDao.findListForMotherLangRecommend(user.getId(), studyLangs, myLangs);
...
//					.addEntity(Item.class).list();
			List<Item> popular_items = itemDao.findListForPopularItems(user.getId(), studyLangs, myLangs, len);
	...
	public void searchUserRecommendedItems(Users user) {
		...
		for (Item item : results) {
			...
			iq.setId(KeyGenerateUtil.generateIdUUID());
			this.itemQueueDao.insert(iq);
		}
	}
...
//			dc.add(Restrictions.eq("disabled", 0));
			List<LanguageAbility> las = this.languageAbilityDao.findListByAuthorId(user.getId());
			for (LanguageAbility la : las) {
				la.setDisabled(1);
				this.languageAbilityDao.update(la);
			}
...
//			}
//			
			Map<String, Integer> lan_map = languageAbilityDao.findMapForLanMap(user.getId());

			if (lan_map.size() == 0)
				continue;

			List<Map<String, Object>> stateMapList = languageAbilityDao.findMapListForStateMap(user.getId());
			Map<String, Map<Integer, Integer>> state_map = new HashMap<String, Map<Integer, Integer>>();
			for(Map<String, Object> rmap: stateMapList){
				String lan_id = (String)rmap.get("language_id");
				Long num = (Long)rmap.get("count");
				Integer state = (Integer)rmap.get("answerstate");
...
//			}
			...
			for (String key : keys) {
				Language lan = this.languageDao.findById(key);
				if (lan == null)
					continue;
				...
				ability.setAuthorId(user.getId());
				ability.setCreateTime(new Date());
				ability.setTotaltimes(total);
				ability.setRighttimes(right);
				ability.setLanguageId(lan.getId());
				ability.setDisabled(0);
				ability.setAbility((double) right / total);
				ability.setId(KeyGenerateUtil.generateIdUUID());
				this.languageAbilityDao.insert(ability);
			}
