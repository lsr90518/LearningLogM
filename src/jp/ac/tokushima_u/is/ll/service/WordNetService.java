...
import jp.ac.tokushima_u.is.ll.dao.ItemRelationDao;
import jp.ac.tokushima_u.is.ll.dao.ItemTitleDao;
import jp.ac.tokushima_u.is.ll.dao.LanguageDao;
import jp.ac.tokushima_u.is.ll.dao.WordNetDao;
...
public class WordNetService {
	 ...
	 * @return 英単語のリスト構造
	 */
	public List<Map<String, Object>> getSynonym(String word, String userid) {
...
//		.setParameter("word", word).setParameter("author_id", userid).list();
		...
		return wordNetDao.findListNearSynonymAndLearningFlag(word, userid);
	}
...
//		return session.createSQLQuery(sql).addEntity(Word.class).setParameter("word", word).list();
		return this.wordNetDao.findListNearSynonym(word);
	}
...
//		return session.createSQLQuery(sql).addEntity(Word.class).setParameter("synset", synset).list();
		return this.wordNetDao.findListNearSynonymBySynset(synset);
	}
...
//				.setParameter("word", word).list();
		return this.wordNetDao.findListSenseListByWord(word);
	}
	/**
	 * getSynset
	 * 単語のSynsetを得る
...
//				.setParameter("word", word).list();
		return this.wordNetDao.findListSynsetDefByWord(word);
	}
...
//			.setParameter("synset", synset).list();
		return this.wordNetDao.findListSynsetDefBySynset(synset);
	}
	 ...
	 */
	public List<Map<String, Object>> getSynlink(String synset) {
//		String sql = "SELECT DISTINCT " +
//				"	{SL.*}, {W.*} " +
...
//				"	ORDER BY SL.id,W.wordid";
		...
		return this.wordNetDao.findSynlinkAndWordBySynset(synset);
//		Session session = this.sessionFactory.getCurrentSession();
//		return session.createSQLQuery(sql)
...
//				.setParameter("synset", synset).list();
	 ...
	 * @return 英単語のリスト構造
	 */
	public List<Map<String, Object>> getSynsetByLink(String word, String link, String userid) {
//		String sql = "SELECT DISTINCT "
...
//			+"	ORDER BY SY.id ASC"
//			+"	";//LIMIT 0, 20
		return this.wordNetDao.findListSynsetByLink(word, link, userid);
//		Session session = this.sessionFactory.getCurrentSession();
//		return session.createSQLQuery(sql)
...
//				.setParameter("word", word).setParameter("link", link).list();
		return this.wordNetDao.findSynsetByLink(word, link);
	}
...
//		return session.createSQLQuery(sql).addEntity(Xlink.class).setParameter("word", word).list();
		return this.wordNetDao.findListXlinkSUMOByWord(word);
	}
...
//		return session.createSQLQuery(sql).addEntity(Synlink.class).setParameter("synset", synset).list();
		return this.wordNetDao.findListSynlinkHypernymBySynset(synset);
	}
	 ...
	 * @return Itemリスト構造
	 */
	public List<Map<String,Object>> getRelatedWordRAW(Item item, int page, int perpage) {
		String name = getEnTitle(itemTitleDao.findListByItem(item.getId()));
		
		/*
		 * return synset, synset0, name, dist
		 */
		return this.wordNetDao.findListRelatedItemRAWInWordMode(name, page, perpage, 5, item.getId());
...
//				.list();
	...
	public List<Map<String, String>> getRelatedWord(Item item, int page, int perpage) {
		List<Map<String,Object>> list = getRelatedWordRAW(item, page, perpage);
		String name = getEnTitle(itemTitleDao.findListByItem(item.getId()));
		...
		for (Map<String, Object> data : list) {
			...
			if (!name.equals(title)) {
				...
				infomap.put("synset", (String)data.get("synset"));
				infomap.put("synset0", (String)data.get("synset0"));
				infomap.put("name", (String)data.get("name"));
				infomap.put("dist", ((Byte)data.get("dist")).toString());
...
//		return session.createSQLQuery(sql).addEntity(Synlink.class).setParameter("word", word).list();
		return this.wordNetDao.findListSynlinkNounSynsetByWord(word);
	}
