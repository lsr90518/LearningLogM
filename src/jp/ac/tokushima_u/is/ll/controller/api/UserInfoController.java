...
//				model.addAttribute("userinfo", userinfo);				
		...
		if (user.getDefaultCategory() != null){
			userinfo.setDefcategory(user.getDefaultCategory());
		}

		userinfo.setNickname(user.getNickname());
		List<Category> categoryList = categoryService.findListByUserId(user.getId());
		...
		for(Language lan:languageService.findMyLangs(user.getId())){
			mylans.put(lan.getId(), lan.getName());
		};
		userinfo.setMylans(mylans);

		Map<String,String>studylans = new HashMap<String,String>();
		for(Language lan:languageService.findStudyLangs(user.getId())){
